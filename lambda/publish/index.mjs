...
export const handler = async (event, context) => {
  ...
  try {
    ...
    if (!own.Item) {
      log.warn("publish: workload not found", { id });
    ...
    const existingArchive = await ddb.send(new GetCommand({
      ...
      ProjectionExpression: "ownerSub, publishedAt",
    }));
    if (existingArchive.Item && existingArchive.Item.ownerSub !== sub) {
      log.warn("publish: id collision with another owner", {
        id,
        existing_owner: existingArchive.Item.ownerSub,
      });
      return forbidden("Workload id already published by another user");
    }

    const isFirstPublish = !existingArchive.Item;
    ...
    log.info("publish: archived", { id, isFirstPublish, ownerEmail: email });
